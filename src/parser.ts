...
export class TorrentsPageParser {
  private typeRe = /<a href=".*\/torrents\.php\?tipus=(.*?)"><img src=".*" class="categ_link" alt=".*" title=".*">/g;
  private idNameRe = /<a href=".*?" onclick="torrent\(([0-9]+)\); return false;" title="(.*?)">/g;
  private dateTimeRe = /<div class="box_feltoltve2">(.*?)<br>(.*?)<\/div>/g;
  private sizeRe = /<div class="box_meret2">(.*?)<\/div>/g;
  private notFoundRe = /<div class="lista_mini_error">Nincs találat!<\/div>/;
  private seedRe = /<div class="box_s2"><a class="torrent" href=".*">([0-9]+)<\/a><\/div>/g;
  private leechRe = /<div class="box_l2"><a class="torrent" href=".*">([0-9]+)<\/a><\/div>/g;
  private currentPageRe = /<span class="active_link"><strong>(\d+).*?<\/strong><\/span>/;
  private lastPageRe = /<a href="\/torrents\.php\?oldal=(\d+)[^>]*><strong>Utolsó<\/strong><\/a>/;

  static getKey(data: string): string {
    const m = data.match(/<link rel="alternate" href=".*?\/rss.php\?key=(?<key>[a-z,0-9]+)" title=".*"/);
    const key = (m && (m as any).groups?.key) || null;
    if (key) return key;
    throw new Error('cant read user key from page');
  }

  private _all(re: RegExp, data: string): string[] {
    const out: string[] = [];
  ...
  private _all2(re: RegExp, data: string): Array<[string, string]> {
  ...
  getItems(data: string) {
    ...
    return ids.map((id, i) => ({
      ...
      type: (SearchParamType as any)[types[i] as any] ?? types[i],
      seed: seed[i] ?? '0',
      leech: leech[i] ?? '0',
    }));
  }
...
export class TorrenDetailParser {
  private typeRe = new RegExp(
    '<div class="dd"><a title=".*?" href=".*?torrents.php\\?csoport_listazas=' +
      '(?<category>.*?)">.*?</a>.*?<a title=".*?" href=".*?torrents.php\\?tipus=' +
      '(?<type>.*?)">.*?</a></div>'
  );
  private dateRe = /<div class="dd">(?<date>[0-9]{4}\-[0-9]{2}\-[0-9]{2}\ [0-9]{2}\:[0-9]{2}\:[0-9]{2})<\/div>/;
  private titleRe = /<div class="torrent_reszletek_cim">(?<title>.*?)<\/div>/;
  private sizeRe = /<div class="dd">(?<size>[0-9,.]+\ [K,M,G,T]{1}iB)\ \(.*?\)<\/div>/;
  private peersRe =
    /div class="dt">Seederek:<\/div>.*?<div class="dd"><a onclick=".*?">(?<seed>[0-9]+)<\/a><\/div>.*?<div class="dt">Leecherek:<\/div>.*?<div class="dd"><a onclick=".*?">(?<leech>[0-9]+)<\/a><\/div>/s;

  getItem(data: string) {
    const tMatch = this.typeRe.exec(data);
    if (!tMatch || !tMatch.groups) throw new Error('Type pattern not found');
...
export class RssParser {
  private idRe = /<source url=".*?\/rss_dl.php\/id=(?<id>[0-9]+)\/key\=.[a-z,0-9]+">/g;
  getIds(data: string): string[] {
    const out: string[] = [];
    let m: RegExpExecArray | null;
    const r = new RegExp(this.idRe.source, this.idRe.flags);
    while ((m = r.exec(data))) {
      if (m.groups?.id) out.push(m.groups.id);
    }
    return out;
  }
}
...
export class ActivityParser {
  ...
  getParams(data: string): Array<[string, string, string, string, string, string, string, string]> {
    ...
    for (const re of this.patterns) {
      ...
      let m: RegExpExecArray | null;
      const r = new RegExp(re.source, re.flags);
      while ((m = r.exec(data))) if (m[1] !== undefined) items.push(m[1]);
      out.push(items);
    }
    const zipped: any[] = [];
...
export class RecommendedParser {
  private recRe = /<a href=".*?torrents.php\?action=details\&id=(.*?)" target=".*?"><img src=".*?" width=".*?" height=".*?" border=".*?" title=".*?"\/><\/a>/g;
  getIds(data: string): string[] {
    const out: string[] = [];
    let m: RegExpExecArray | null;
    const r = new RegExp(this.recRe.source, this.recRe.flags);
    while ((m = r.exec(data))) if (m[1] !== undefined) out.push(m[1]);
    return out;
  }
}
